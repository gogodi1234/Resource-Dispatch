...
import { LayoutDashboard, List, Map as MapIcon, Box, Activity, AlertCircle, Users } from 'lucide-react'
...
function App() {
  ...
  const calculateStatus = useCallback((startDate: string, deadline: string, currentStatus?: Project['status']): Project['status'] => {
    // If it's already manually set to 'on-hold', keep it.
  ...
  const stats = useMemo(() => ({
    ...
    unassigned: projects.filter(p => p.assignedPersonnel.length === 0).length,
    highWorkload: personnelWithWorkload.filter(p => p.workload >= 80).length
  }), [projects, personnelWithWorkload]);
  ...
  const handleAddOrUpdateProject = (project: Project) => {
    setProjects(prev => {
      ...
      const finalProject = { ...project, status: calculateStatus(project.startDate, project.deadline, project.status) };
      if (exists) {
        return prev.map(p => p.id === project.id ? finalProject : p);
      }
      return [finalProject, ...prev];
    });
    if (selectedProject?.id === project.id) setSelectedProject(prev => prev ? ({ ...prev, ...project, status: calculateStatus(project.startDate, project.deadline, project.status) }) : null);
  };

  const handleToggleOnHold = (projectId: string) => {
    setProjects(prev => prev.map(p => {
      if (p.id !== projectId) return p;
      const newOnHold = p.status !== 'on-hold';
      const status = newOnHold ? 'on-hold' : calculateStatus(p.startDate, p.deadline);
  ...
  return (
    <div className="app-container" style={{ minHeight: '100vh', backgroundColor: '#f8fafc', fontFamily: 'Inter, system-ui, sans-serif' }}>
      <header style={{ backgroundColor: '#fff', borderBottom: '1px solid #e5e7eb', padding: '0.75rem 2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 40 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <LayoutDashboard color="#4F46E5" size={24} />
          <h1 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 700, color: '#111827' }}>Atlas Deployment Hub</h1>
        </div>
        <div style={{ fontSize: '0.875rem', color: '#6b7280' }}>System Date: <strong>2026-03-05</strong></div>
      </header>

      <main style={{ padding: '1.5rem 2rem', maxWidth: '1800px', margin: '0 auto' }}>
        <div style={{ display: 'flex', gap: '1.5rem', marginBottom: '1.5rem' }}>
          <StatCard icon={Box} label="Total Projects" value={stats.total} color="#4F46E5" />
          <StatCard icon={Activity} label="Active Projects" value={stats.active} color="#10B981" />
          <StatCard icon={AlertCircle} label="Unassigned" value={stats.unassigned} color="#F59E0B" />
          <StatCard icon={Users} label="High Workload" value={stats.highWorkload} color="#EF4444" />
        </div>
        ...
        <FilterBar 
          ...
          statusOptions={['planning', 'ongoing', 'completed', 'on-hold']}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 400px', gap: '1.5rem', alignItems: 'start' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            <div style={{ backgroundColor: '#fff', padding: '1.25rem', borderRadius: '12px', border: '1px solid #e5e7eb' }}>
              ...
              {viewMode === 'map' ? <WorldMap projects={filteredProjects} onMarkerClick={setSelectedProject} /> : <ProjectTable projects={filteredProjects} onProjectClick={setSelectedProject} />}
            </div>
            <TimelineView projects={filteredProjects} personnel={filteredPersonnel} />
          </div>

          <aside style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', position: 'sticky', top: '5.5rem' }}>
            {selectedProject ? (
              <ProjectDetails 
      ...
      <AddModal 
        ...
        onAddPersonnel={handleAddOrUpdatePersonnel} 
      />
    </div>
  )
}
