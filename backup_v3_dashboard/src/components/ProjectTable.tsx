...
const ProjectTable: React.FC<ProjectTableProps> = ({ projects, onProjectClick, selectedProjectId }) => {
  ...
  const getStatusStyle = (status: Project['status']) => {
    ...
    switch (status) {
      ...
      case 'delay': return { ...base, backgroundColor: '#fee2e2', color: '#991b1b' };
      case 'on-hold': return { ...base, backgroundColor: '#fff7ed', color: '#9a3412' };
        ...
        <tbody>
          {sortedProjects.map(project => {
            ...
            return (
              ...
              >
                <td style={{ padding: '12px 16px', fontSize: '13px', fontWeight: isSelected ? 700 : 600, color: isSelected ? '#4F46E5' : '#111827' }}>{project.customer}</td>
